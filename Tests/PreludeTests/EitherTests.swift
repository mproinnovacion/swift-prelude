...
class EitherTests: XCTestCase {
	...
	func testAlt() {
		...
		let finalLeft = left.orElse(left2)
		
		XCTAssert(finalLeft.isLeft)
		XCTAssert(finalLeft.left == -2)
		
		let finalRight = Either.alternative(left, right)
		
		XCTAssert(finalRight.isRight)
		XCTAssert(finalRight.right == 1)
		
		
		let finalRightR = right.orElse(right2)
	...
	func testAltDefault() {
		...
		let def = left.default(2)
		XCTAssert(def == 2)
		
		let r = right.default(2)
		XCTAssert(r == 1)
	}
	...
	func testResult() {
		let left = Either<TestError, Int>.left(.unknown)
		let right = Either<TestError, Int>.right(1)
		...
		XCTAssertEqual(
			left,
			Either.from(.failure(.unknown))
		)
		
		XCTAssertEqual(
			right,
			Either.from(.success(1))
		)
	}
}
